...
def tx_template(N, D, upsamp_rate):
    ...
    x_0 = x0 + j * xq0
    ############################################################
    x_cx = np.multiply(x_0, win) # add window
...
def rx_sim(N):
    ...
    win=1
    Amp = -1
    phi_init1 = 23/180#-105.0 / 180.0 * np.pi
    phi_init2 = 23/180#-105.0 / 180.0 * np.pi
    dc_offset = 0
    ...
    # Chirp
    y0 =  np.sin(2 * np.pi * (f0 * t + K / 2 * np.power(t, 2)))   # use this for chirp generation
    yq0 = np.sin(2 * np.pi * (f0 * t + K / 2 * np.power(t, 2)) - np.pi / 2)  # use this for chirp generation
    ...
    for i in range (0,N-1):
        ...
        while (f0 + K / 2 * t[i] - freq_delay > f1):
            freq_delay = freq_delay + bw
        y1[i] =  np.sin(2 * np.pi * (f0 + K / 2 * t[i] - freq_delay ) * t[i] - beta * z) + dc_offset  # use this for chirp generation
        yq1[i] = np.sin(2 * np.pi * (f0 + K / 2 * t[i] - freq_delay ) * t[i] - beta * z - np.pi / 2)   # use this for chirp generation
