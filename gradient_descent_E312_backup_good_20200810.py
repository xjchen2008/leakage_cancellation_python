...
def upsampling(x, upsamp_rate):
    ...
    #plt.show()
    x_upsamp = np.reshape(xx, N*upsamp_rate) # change back to 1-D
    return x_upsamp
...
def tx_template(N, D):
    ...
    for i in range (D):
        ...
        x_cx_delay[:,i] = np.roll(x_cx_upsamp, 1*i) # here  #x_cx_delay[:,i] = np.roll(x_cx, 20*i) # here

    x_cx_delay = downsampling(x_cx_delay,downsamp_rate)
