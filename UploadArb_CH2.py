...
def UploadArb(x_canc=np.zeros(5000), SOUR = 'SOUR2'):
    #parse arguments
    ...
    if args.macro:
        ...
        with open(macroFile, 'w') as f:
            ...
            f.write("SOURCE2" + SOUR[-1] + ":FUNCtion:ARBitrary \"INT:\\remoteAdded\\"+name+".arb\"\n")

if __name__ == '__main__':
    #y = readcsv('output_cal_antenna_ch2.csv')  # a pre-record loopback waveform.
    x = coe.y_cx.real
    N = len(x)
    win = np.blackman(N) #np.hanning(N) #np.hamming(N) #np.blackman(N)
