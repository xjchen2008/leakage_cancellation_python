...
def gd(theta, rx_error_sim, X1):
    ...
    cost_history = 10 * np.log10(cal_cost(X1, rx_error_sim))
...
def main(theta, N=4096, D=2, rx_error_sim=np.zeros([4096, 1])):
    ...
    # X = np.matrix(X0)
    X1 = np.load(
        'tx_template_order9_delay1_upsamp100_28MHz_x1.npy')  # Generate X for accelerating the gradient descent.
    X1 = np.matrix(X1)
    # X00 = np.matrix(X0)
    # print(X00- X)
    # X = X00
    ...
    print(theta_cx_in.shape, rx_error.shape, X1.shape)
    #theta_cx_out, y_hat, cost_history = gd(theta_cx_in, rx_error, X1)
...
if __name__ == "__main__":  # Change the following code into the c++
    ...
    for i in range(0, M, 1):  # this for loop sweeps the received signal fractional time delay
        delay_tap = 0  # M
        y = y0[:, 0]  # rx_sim(N, upsamp_rate_rx, delay_tap) ##
        y_withnoise = y + s
        y = y  # y_withnoise
