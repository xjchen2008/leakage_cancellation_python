...
def tx_template(N, D, upsamp_rate):
    ...
    # Chirp
    x0 = 0.9 * np.sin(2 * np.pi * (f0 * t + K / 2 * np.power(t, 2)))  # use this for chirp generation
    xq0 = 0.9 * np.sin(2 * np.pi * (f0 * t + K / 2 * np.power(t, 2)) - np.pi / 2)  # use this for chirp generation
    ...
    for i in range (D):
        x_cx_order[:,i] = np.power(abs(x_cx),2*(i+1)-1) * x_cx
