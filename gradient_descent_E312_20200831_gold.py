...
def gd(theta, rx_error_sim, X, X1):
    # gradient decent
    N = len(rx_error_sim)
    M = len(X1)/2#N/2
...
def tx_template(N, D, upsamp_rate):
    ...
    x_upsamp = upsampling(np.reshape(x_cx_gd, (N, 1)), upsamp_rate)  # step 1: up-sampling
...
def main(theta, N=4096, D=2, rx_error_sim=np.zeros([4096, 1])):
    ...
    X0 = np.load('tx_template_order9_delay1_upsamp100_28MHz.npy')  # tx_template(N,D, upsamp_rate) # Step 1: up-sampling and time delay#
    # X1 = zca_whitening_matrix(X0) # step 2: whitening
    # X2 = zca_whitening_matrix(X1)
    # X3= zca_whitening_matrix(X2)
    #time.sleep(1)
    X = np.matrix(X0)
    X1 = np.load('tx_template_order9_delay1_upsamp100_28MHz_x1.npy') # Generate X for accelerating the gradient descent.
    X1 = np.matrix(X1)
    # X00 = np.matrix(X0)
    # print(X00- X)
    # X = X00
    ...
    print(theta_cx_in.shape, rx_error.shape, X.shape)
    theta_cx_out, y_hat, cost_history = gd(theta_cx_in, rx_error, X, X1)
    #theta_cx_out, y_hat, cost_history = gd(theta_cx_in, rx_error_sim, X, X1) #uncomment this line out when using simulated received signal
...
if __name__ == "__main__":  # Change the following code into the c++
    ...
    upsamp_rate_tx = 100 # Update this for real tx template
    upsamp_rate_rx = 1000

    M = 1
    y0 = tx_template(N, D, upsamp_rate_tx)  # rx_sim(N)
