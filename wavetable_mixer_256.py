...
# f0=-28e6#Start Freq
# f1=28e6#End freq
f0 = 3e6#-14e5  # Start Freq
f1 = 13e6 # fs/2=1/2*N/T#End freq
k = (f1 - f0) / T
phi0 = -(4999+1) * numpy.pi / 10000  # Phase
f = numpy.linspace(0, fs-1, N)
freq = numpy.fft.fftfreq(N1, d=1. / fs)
...
PC = 20*numpy.log10(abs(numpy.fft.fft(A*numpy.conj(B))))
...
plt.plot(t, numpy.real(y_cx_delay), '*-', t, numpy.imag(y_cx_delay), 'r*-')
plt.grid(True)

plt.figure(2)
plt.plot( numpy.fft.fftshift(PC))
plt.figure(3)
