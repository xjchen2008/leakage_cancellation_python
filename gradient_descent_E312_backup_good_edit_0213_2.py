...
def rx_sim(N):
    ...
    win=1
    Amp = 1
    phi_init1 = -90.0 / 180.0 * np.pi
    phi_init2 = -90.0 / 180.0 * np.pi
    dc_offset = 0
    ...
    y = y0 + j * yq0
    y = np.multiply(y, win)
    y_cx_delay = np.roll(y, 0)#np.concatenate((y[N-100:], y[:N-100]), axis=0)

    y_cx_delay = np.reshape(y_cx_delay, [N, 1])
    return y_cx_delay
...
def main(theta, N=4096, D=2, rx_error_sim = np.zeros([4096, 1])):
    ...
    X0 = tx_template(N,D)
    X_whitening = X0#zca_whitening_matrix(X0)
    X = X_whitening
    # Gradient Decentg
    rx_error = np.reshape(read_last_pulse("usrp_samples_loopback.dat",N), [N,1])

    theta_cx_in = 1j * 1e-10 * np.ones([D, 1])  # Set a small complex number close to 0 for record imag part of theta.
    for i in range(D):
...
if __name__ == "__main__": # Change the following code into the c++
    ...
    # non-equispaced delay
    rate = 2
    y_up = upsampling(y, rate)
    y_up_delay = np.roll(y_up,1)
    y_delay_0p5samp = downsampling(y_up_delay,rate)
    y_delay_1samp = np.roll(y,1)
    y_nodelay = y
    '''
    plt.plot(np.linspace(0,1, y_nodelay.shape[0]), y_nodelay ,'ko')
    plt.plot(np.linspace(0,1, y_delay_1samp.shape[0]), y_delay_1samp, 'r*')
    plt.plot(np.linspace(0, 1, y_delay_0p5samp.shape[0]), y_delay_0p5samp, 'yo-')
    plt.show()
    '''
    y= y
