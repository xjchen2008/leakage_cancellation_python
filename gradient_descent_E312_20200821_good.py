...
def gd(theta, rx_error_sim, X):
    ...
    theta = theta - (1.0 / m) * eta * np.dot(np.conj(X[N/2-1000:N/2+1000].T), rx_error_sim[N/2-1000:N/2+1000,:] / c2) # just take a portion of the signal
...
def tx_template(N, D, upsamp_rate):
    ...
    x_cx_order = x_upsamp #j * np.ones([N * upsamp_rate])

    k=0 # delay tap
    order_idx = 1
    order = 1
    digital_filter_length = 1
    for i in range(D):
