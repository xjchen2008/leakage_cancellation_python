...
def tx_template(N, D, upsamp_rate):
    ...
    for i in range (D):
        ...
        x_cx_order[:, i] = np.power(abs(x_upsamp), 2 * (i + 1) - 1) * x_upsamp

    #x_cx_delay = downsampling(x_cx_delay, upsamp_rate)
    #X = np.matrix(x_cx_delay)
    x_cx_order = downsampling(x_cx_order, upsamp_rate)

    X1 = zca_whitening_matrix(x_cx_order) # whitening
    X2 = zca_whitening_matrix(X1)
    X3 = zca_whitening_matrix(X2)
    X4 = zca_whitening_matrix(X3)
    X5 = zca_whitening_matrix(X4) # whitening
    X6 = zca_whitening_matrix(X5)
    X7 = zca_whitening_matrix(X6)
    X = zca_whitening_matrix(X7)

    #X = np.matrix(x_cx_order)
    
    np.save('tx_template_order9_delay1_upsamp100_28MHz.npy', X)
