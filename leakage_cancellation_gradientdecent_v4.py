...
def main():
    ...
    y0 = -0.13*np.sin(2 * np.pi * fs / N * t + phi_init)  # + numpy.sin(4*numpy.pi*fs/N*t)# just use LO to generate a LO. The
    yq0 = -0.13*np.sin(2 * np.pi * fs / N * t - np.pi / 2 + phi_init)  # + numpy.sin(4*numpy.pi*fs/N*t-numpy.pi/2)
    ...
    #H = np.matrix(np.transpose([psi_orth_delay_0, psi_orth_delay_1]))
    ...
    np.random.seed(100)
    theta = np.random.randn(H.shape[1],1) # cannot use zeros for theta initial value, wont converge.
    lr = 1
    n_iter = 4000
    const = 1
    X_sim =  tx_sig_sim
    X_sim = X_sim[np.newaxis].T
    X = tx_sig_sim#tx_sig_sim#tx_sig#rx_avg
    y = rx_avg#tx_sig#rx_avg#tx_sig#rx_avg-tx_sig
    X = X[np.newaxis].T
    y = y[np.newaxis].T
    #y0 = y
    #theta,cost_history,theta_history = gd.gradient_descent(X,y,theta, y0,lr, n_iter)
    ...
    x_canc_gd_sim  = np.squeeze(np.array(X_sim.dot(theta))) # If don't count the response of tx channel, just send ideal waveform to ettus Tx. After tx chain at output, the singal convolute with channel,  x_can * h = y_hat, y_hat - y = error
    print(theta)
    ...
    # save updated x_canc
    x_canc=np.zeros(2*N)
    x_canc_gd_sim_update = x_canc_gd_sim / max(abs(x_canc_gd_sim)) * 32767*8/10 # use the simulated or ideal waveform avoid double connting the tx channel effect.
    for i in range(0, N):
        x_canc[2 * i + 1] = np.imag(x_canc_gd_sim_update[i])  # tx signal
        x_canc[2 * i] = np.real(x_canc_gd_sim_update[i])  # tx signal
