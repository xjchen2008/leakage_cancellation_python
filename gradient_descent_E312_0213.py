...
def gd(theta, rx_error_sim, X, D):
    # gradient decent
    eta = 1.0/D# 1  # learning rate
    m = len(rx_error_sim)
...
def tx_template(N, D, upsamp_rate):
    ...
    tc = upsamp_rate * N / fs  # T=N/fs#Chirp Duration
    t = np.linspace(0, tc, upsamp_rate*N)
    f0 = -14e6  # Start Freq
    ...
    #x_cx = np.reshape(x_cx,[N,1])

    #x_cx_upsamp = upsampling(x_cx, upsamp_rate)
    # Dimension of X matrix.Total_delay_tap
    x_cx_delay = j*np.ones([upsamp_rate * N,D])
    for i in range (D):
        #x_basis = np.roll(x_cx_upsamp, 0 + i)  # here
        #if i > 0:
        #    x_basis[:i] = 0
        #x_cx_delay[:,i] = np.roll(x_cx[:,0], 1*i) # here  #x_cx_delay[:,i] = np.roll(x_cx, 20*i) # here
        x_cx_delay[:, i] = np.roll(x_cx[:], -1 * i)
    #x_cx_delay[:, 0] = np.roll(x_cx_upsamp, 0)#np.zeros(N)
    #x_cx_delay[:, 1] = np.roll(x_cx_upsamp, 100)  # x_cx
...
def main(theta, N=4096, D=2, upsamp_rate=10, rx_error_sim = np.zeros([4096, 1])):
    print('D = ', D)
    start = timeit.default_timer()
    ###################################################
    # Generate the X matrix and read rx residual signal
    ###################################################
    # Generate the X matrix
    X0 = tx_template(N, D, upsamp_rate) # including upsampling
    # whitening X matrix
    X_whitening = zca_whitening_matrix(X0)
    # up-sampling X matrix
    X_upsamp = X0# X_whitening # template signal up-sampling
    X = X_upsamp
    # read rx residual signal
    rx_error0 = np.reshape(read_last_pulse("usrp_samples_loopback.dat",N), [N,1])
    # up-sampling rx residual signal
    rx_error_upsamp = upsampling(rx_error0,upsamp_rate) # residual signal up-sampling
    rx_error = rx_error_upsamp
    # Read the parameter vector history from function input
    ...
    # Gradient Decent
    ##################
    #theta_cx_out, y_hat_upsamp, cost_history = gd(theta_cx_in, rx_error, X)
    theta_cx_out, y_hat_upsamp, cost_history = gd(theta_cx_in, rx_error_sim, X, D) #uncomment this line out when using simulated received signal

    #######################
    # Prepare data for FPGA
    #######################
    y_hat = downsampling(y_hat_upsamp, upsamp_rate) # cancellation signal down-sampling for DAC fs
    save_tx_canc('usrp_samples4096_chirp_28MHz_fixed_delayed.dat', N, y_hat) # add FGPA tx delay inside this function
    theta_real_out = []
    theta_imag_out = []
    theta_tuple_out = ()
    for i in range(D):
        theta_real_out.append(float(np.matrix.tolist(np.real(np.matrix(theta_cx_out.T)))[0][i]))
        theta_imag_out.append(float(np.matrix.tolist(np.imag(np.matrix(theta_cx_out.T)))[0][i]))
...
if __name__ == "__main__": # Change the following code into the c++
    ...
    # Initial values for theta, y and y_hat
    theta0 = 0.0#+0.1j# *np.random.randn(1, 1) + 0.1j  # parameter to learn
    theta = np.repeat(theta0, upsamp_rate * D)  # this should be a D * 1 column vector
    theta_tuple = ()
    theta_tuple = array2tuple(theta, theta.shape[0])

    s = np.random.normal(mu, sigma, [N*upsamp_rate, 1])
    y0 = rx_sim(N)
    y = upsampling(y0, upsamp_rate) # received signal up-sampling
    y_withnoise = y + s
    y = y#y_withnoise
    ...
    start = timeit.default_timer()
    for itt in range(30):
        y_hat = upsampling(y_hat, upsamp_rate)  # cancelation signal up-sampling
        #plt.plot(y, '*-')
        #plt.plot(y_hat,'*-')
        #plt.show()
        rx_error_sim = y_hat + y  # uncomment this line out when using simulated received signal
        theta_tuple, y_hat = main( theta_tuple, N, upsamp_rate * D, upsamp_rate,  rx_error_sim) # uncomment this line out when using simulated received signal

        #theta_tuple = main(theta_tuple, N, upsamp_rate* D)
        #print(itt)
