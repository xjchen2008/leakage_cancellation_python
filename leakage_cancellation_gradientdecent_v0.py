...
import readbin
import gradient_descent_v0 as gd
...
x_canc_gd = np.squeeze(np.array(np.dot(X,(theta)))) # as an array not matrix
print(theta)
#print('Theta0:          {:0.3f},\nTheta1:          {:0.3f}'.format(np.abs(theta[0][0]),np.abs(theta[1][0])))
...
PC = PulseCompr(rx = x_canc_gd.T,tx=tx_sig,win=np.blackman(N))
plt.figure()
