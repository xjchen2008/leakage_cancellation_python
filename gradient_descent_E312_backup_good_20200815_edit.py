...
def main(theta, N=4096, D=2, rx_error_sim = np.zeros([4096, 1])):
    ...
    X = np.load('tx_template_order_28MHz.npy') #tx_template(N,D, upsamp_rate) # Step 1: up-sampling and time delay#
    X = np.matrix(X)
