...
def tx_template(N, D, upsamp_rate):
    ...
    #######################
    c = 3e8
    Lambda = c * tc  # chirp wavelength
    beta = 2 * np.pi / Lambda
    gamma = (f1 - f0) / Lambda  # check this
    bw = f1 - f0
    ...
    for idx in range(0, D):
        ...
        for i in range(0, N - 1):
            while (f0 + K / 2 * t[i] - freq_delay < f0):
                freq_delay = freq_delay - bw
            while (f0 + K / 2 * t[i] - freq_delay > f1):
                freq_delay = freq_delay + bw
            x1[i] = np.sin(
                2 * np.pi * (f0 + K / 2 * t[i] - freq_delay) * t[i] - beta * z)  # use this for chirp generation
            xq1[i] = np.sin(
                2 * np.pi * (f0 + K / 2 * t[i] - freq_delay) * t[i] - beta * z - np.pi / 2)  # use this for chirp generation
        x_cx_delay[:, idx] = x1 + j * xq1
        z = z + z_delta
    ##################################
    ...
    for i in range (D):
        ...
        #x0 = 1 * np.cos(2 * np.pi * (f0 * t + K / 2 * np.power(t, 2)) + phi_init)  # use this for chirp generation
...
def rx_sim(N):
    ...
    # Sine wave
    #y0 = Amp * np.cos(2 * np.pi * fs / N * t)
    #yq0 = Amp * np.sin(2 * np.pi * fs / N * t - np.pi / 2)

    #y0 = Amp * np.cos(2 * np.pi * fs / N * t + phi_init1) + dc_offset # + numpy.sin(4*numpy.pi*fs/N*t)# just use LO to generate a LO. The
    ...
    y = y0 + j * yq0
    y = y*np.exp(j*np.pi/2)
    plt.plot(y)
    # Chirp
    phi_init3 = 0.0/180*np.pi
    phi_init4 = 0.0/180*np.pi
    y01 = Amp * np.sin(phi_init3 + 2 * np.pi * (f0 * t + K / 2 * np.power(t, 2))) + dc_offset  # use this for chirp generation
    yq01 = Amp * np.sin(phi_init4 + 2 * np.pi * (f0 * t + K / 2 * np.power(t, 2)) - np.pi / 2) + dc_offset  # use this for chirp generation
    y1 = y01 + j * yq01
    y1 = np.multiply(y1, np.exp(-j*0))
    plt.plot(y1)
    plt.show()
